...
// Keep this as a named function so it can also be bound directly whenever Dash
// replaces the table subtree after an optimized refresh.
...
// Delegation handles normal page events. A direct listener is also installed
// on each newly rendered table below; this is intentionally redundant only at
// the event-routing level, not for a single click (the direct handler marks
// the event as handled before it reaches this listener).
document.addEventListener('click', function(e) {
    if (e.__taskTableInteractionHandled) return;
    e.__taskTableInteractionHandled = true;
    handleTaskTableClick(e);
}, true);

function bindTaskTableInteractions(table) {
    if (!table || table.dataset.taskTableInteractionsBound === 'true') return;
    table.dataset.taskTableInteractionsBound = 'true';
...
function refreshTaskTableInteractions() {
    const container = document.getElementById('task-table-container');
    if (!container) return;
    const table = container.querySelector('table');
    if (!table) return;
    bindTaskTableInteractions(table);
    applyHiddenColumns();
}

// Dash replaces task-table-container children instead of updating individual
// rows. Reinstall the listener after each replacement; observing child-list
// changes cannot recurse when highlighting merely changes CSS classes.
function installTaskTableInteractionObserver() {
    const container = document.getElementById('task-table-container');
    if (!container || window.__taskTableInteractionObserver) return;
    const sync = function() { window.requestAnimationFrame(refreshTaskTableInteractions); };
    window.__taskTableInteractionObserver = new MutationObserver(sync);
    window.__taskTableInteractionObserver.observe(container, {childList: true, subtree: true});
    refreshTaskTableInteractions();
}
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', installTaskTableInteractionObserver, {once: true});
} else {
    installTaskTableInteractionObserver();
}
