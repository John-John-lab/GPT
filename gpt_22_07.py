...
import os, json, time, threading, queue, uuid, shutil, glob, hashlib, re, functools, sys, bisect, math, gzip
...
# Vectorize only oscillator condition/window matching. The path-dependent trade
# simulator remains sequential and authoritative. Disable for an immediate
# scalar-parity rollback while comparing a specific historical snapshot.
DYNAMIC_STOCH_MASK_VECTORIZATION_ENABLED = os.environ.get("GPT_DYNAMIC_STOCH_VECTORIZED", "1") == "1"
...
CHART_INCREMENTAL_SCHEMA_VERSION = 2
...
def make_chart_cci_trace(**kwargs):
    """Return the CCI momentum line with an independent SVG fallback."""
...
def compute_chart_cci(high, low, close, period=20, constant=0.015):
    """Return standard Commodity Channel Index values.

    CCI compares typical price with its rolling simple mean and mean absolute
    deviation. Zero-deviation windows remain gaps rather than producing
    infinities, matching the chart's existing null-safe indicator behavior.
    """
    ...
    # Compute every rolling deviation in one NumPy operation. pandas
    # rolling.apply invokes Python once per candle and is noticeably more
    # expensive when CCI participates in compact Previous/Next navigation.
    values = typical_price.to_numpy(dtype="float64", copy=False)
    mean_deviation = pd.Series(np.nan, index=typical_price.index, dtype="float64")
    if len(values) >= period:
        windows = np.lib.stride_tricks.sliding_window_view(values, period)
        deviations = np.mean(np.abs(windows - np.mean(windows, axis=1)[:, None]), axis=1)
...
def build_fast_candle_navigation_payload(task, task_id, symbol, df, current_schema,
    ...
    if len(df) > 1:
        candle_step = int(df["timestamp"].iloc[-1]) - int(df["timestamp"].iloc[-2])
        if candle_step > 0:
...
# Compact navigation responses contain many numeric arrays and compress very
# well. Compress them at the Flask boundary so a stable all-oscillator setup
# spends less time transferring JSON. Browsers transparently decompress it;
# disable with GPT_CHART_RESPONSE_GZIP=0 if a reverse proxy already handles it.
CHART_RESPONSE_GZIP_ENABLED = os.environ.get("GPT_CHART_RESPONSE_GZIP", "1") == "1"
CHART_RESPONSE_GZIP_MIN_BYTES = max(1024, int(os.environ.get("GPT_CHART_RESPONSE_GZIP_MIN_BYTES", "65536")))
...
def trace_chart_dash_response(response):
    ...
    if started_at is not None:
        raw_size = response.calculate_content_length()
        wire_size = raw_size
        gzip_ms = 0
        accepts_gzip = "gzip" in str(request.headers.get("Accept-Encoding") or "").lower()
        content_type = str(response.headers.get("Content-Type") or "").lower()
        can_compress = (
            CHART_RESPONSE_GZIP_ENABLED
            and accepts_gzip
            and response.status_code >= 200 and response.status_code < 300
            and not response.direct_passthrough
            and not response.headers.get("Content-Encoding")
            and "json" in content_type
            and raw_size is not None and raw_size >= CHART_RESPONSE_GZIP_MIN_BYTES
        )
        if can_compress:
            gzip_started = time.perf_counter()
            compressed = gzip.compress(response.get_data(), compresslevel=4)
            gzip_ms = round((time.perf_counter() - gzip_started) * 1000)
            # Only use compression when it is actually beneficial. This also
            # makes the hook safe for unexpectedly high-entropy JSON.
        ...
        interaction_trace(
            ...
            f"bytes={wire_size if wire_size is not None else 'streamed'} "
            f"raw_bytes={raw_size if raw_size is not None else 'streamed'} gzip_ms={gzip_ms}"
        )
    return response
...
function applyLocalToolbarInteraction(button) {
    ...
    // This preference uses session storage, so its DOM label is authoritative
    // after a reload while the in-memory optimistic map starts from defaults.
...
const measureExclusiveButtons = new Set([
    ...
    'toggle-adx-btn', 'toggle-macd-btn', 'toggle-disparity-btn', 'toggle-cci-btn',
...
function uniqueMeasureShapes(shapes) {
    ...
    return (shapes || []).filter(function(shape) {
        if (!shape || shape.name === 'measure_oscillator_range_export' || (shape.type && shape.type !== 'rect') || shape.x0 == null || shape.x1 == null || shape.y0 == null || shape.y1 == null) return false;
...
function clearMeasureOscillatorExportShapes(plot) {
    if (!plot || !window.Plotly) return;
    const shapes = ((plot.layout && plot.layout.shapes) || []).filter(function(shape) {
        return !(shape && shape.name === 'measure_oscillator_range_export');
    });
    window.Plotly.relayout(plot, {shapes: shapes});
}

function addMeasureOscillatorExportShapesForPng(plot) {
    if (!plot || !plot.layout || !plot._fullLayout || !window.Plotly || !window.__taskChartOscillatorRangeEnabled) return;
    const baseCount = Math.max(0, Number(plot.__dashBaseShapeCount || 0));
    const shapes = ((plot.layout && plot.layout.shapes) || []).filter(function(shape) {
        return !(shape && shape.name === 'measure_oscillator_range_export');
    });
    const measureShapes = uniqueMeasureShapes(shapes.slice(baseCount));
    if (!measureShapes.length) return;
    const oscillatorAxes = Object.keys(plot._fullLayout || {}).map(function(key) {
        return plot._fullLayout[key];
    }).filter(function(axis) {
        return axis && axis._id && axis._id !== 'y' && /^y[0-9]+$/.test(axis._id) &&
            axis.visible !== false && Array.isArray(axis.domain) && axis.domain.length === 2;
    });
    if (!oscillatorAxes.length) return;
    measureShapes.forEach(function(measureShape) {
        oscillatorAxes.forEach(function(axis) {
            shapes.push({
                type: 'rect', name: 'measure_oscillator_range_export',
                xref: 'x', yref: 'paper', x0: measureShape.x0, x1: measureShape.x1,
                y0: axis.domain[0], y1: axis.domain[1],
                fillcolor: 'rgba(158,158,158,0.20)',
                line: {color: 'rgba(117,117,117,0.60)', width: 1},
                layer: 'below'
            });
        });
    });
...
function attachNativeMeasureOverlayListeners(plot) {
    ...
    if (typeof plot.on === 'function') {
        ...
        plot.on('plotly_beforeexport', function() { addMeasureOscillatorExportShapesForPng(plot); });
        plot.on('plotly_afterexport', function() { clearMeasureOscillatorExportShapes(plot); });
    }
...
function showNativeMeasureResultAfterMouseup() {
    window.setTimeout(function() {
        ...
        if (xaxis && yaxis && svg && xaxis.d2l && yaxis.d2l && xaxis.l2p && yaxis.l2p) {
            ...
            if (!window.__taskChartOscillatorRangeEnabled) clearMeasureOscillatorExportShapes(plot);
        }
    }, 80);
}
document.addEventListener('mouseup', showNativeMeasureResultAfterMouseup, true);
function requestFullChartFallback(taskId, reason) {
    traceUi('fast chart fallback', {taskId: taskId, reason: String(reason || 'unknown')});
    if (window.dash_clientside && typeof window.dash_clientside.set_props === 'function') {
        window.dash_clientside.set_props('chart-force-full-render-store', {
            data: {task_id: String(taskId || ''), reason: String(reason || 'unknown'), ts: Date.now()}
        });
    }
}
async function applyFastCandleNavigationPayload(rawPayload) {
    ...
    try {
        ...
        if (!plot || !window.Plotly || !Array.isArray(plot.data)) {
            requestFullChartFallback(payload.task_id, 'plot unavailable');
            return false;
        }
        const pendingTaskId = String(window.__gptPendingChartTaskId || '');
        if (pendingTaskId && pendingTaskId !== String(payload.task_id)) {
            traceUi('fast chart payload discarded', {
                taskId: payload.task_id, pendingTaskId: pendingTaskId,
                reason: 'newer chart request is active'
            });
            return false;
        }
        const actualKeys = plot.data.map(function(trace) {
        ...
        const currentSharedX = ((plot.data[0] || {}).x) || [];
        const sameSharedX = currentSharedX.length === payload.shared_x.length &&
            payload.shared_x.every(function(value, index) {
                const current = currentSharedX[index];
                const currentMs = current instanceof Date ? current.getTime() :
                    (Number.isFinite(Number(current)) ? Number(current) : Date.parse(current));
                return currentMs === Number(value);
            });
        ...
        if (root) root.querySelectorAll('[id^="task-chart-measure-label-"], [id^="task-chart-measure-pointer-"], [id^="task-chart-measure-osc-range-"]').forEach(function(node) { node.remove(); });
...
function installFastChartPayloadObserver() {
    const node = document.getElementById('chart-fast-navigation-payload');
    if (!node || node.__gptFastChartObserverInstalled) return false;
    node.__gptFastChartObserverInstalled = true;
    let lastPayload = '';
    const applyCurrent = function() {
        const raw = String(node.textContent || '').trim();
        if (!raw || raw === lastPayload) return;
        lastPayload = raw;
        applyFastCandleNavigationPayload(raw);
    };
    new MutationObserver(applyCurrent).observe(node, {childList: true, characterData: true, subtree: true});
    applyCurrent();
    return true;
}
(function scheduleFastChartObserver() {
    let attempts = 0;
    const timer = window.setInterval(function() {
        attempts += 1;
        if (installFastChartPayloadObserver() || attempts >= 120) window.clearInterval(timer);
    }, 250);
})();
...
document.addEventListener('click', function(e) {
    ...
    if (button.id === 'prev-chart-btn' || button.id === 'next-chart-btn') {
        ...
        window.__gptChartNavigationPending = true;
        window.__gptChartNavigationPendingSince = performance.now();
        ['prev-chart-btn', 'next-chart-btn'].forEach(function(id) {
...
def build_chart_row_layout(indicator_count, stochastic_visible=False, main_full_height=False,
    ...
    if main_full_height or osc_full_height:
        ...
        return (
            [main_pixels] + [indicator_pixels] * count,
...
def build_root_layout():
    ...
    dcc.Store(id="chart-force-full-render-store", data=None),
    # The compact candle payload is rendered as hidden text so the page-shell
    # observer can apply it without relying on version-sensitive inline Dash
    # clientside callbacks.
    html.Div(id="chart-fast-navigation-payload", style={"display": "none"}),
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Button("CCI: Off", id="toggle-cci-btn", title="Toggle Commodity Channel Index momentum (20)", style={
                                "background": "transparent",
                                "color": "black",
                                "border": "1px solid black",
                                "padding": "6px 10px",
...
@app.callback(
    Output("toggle-main-chart-size-btn", "children"),
    Output("toggle-main-chart-size-btn", "style"),
    Input("main-chart-full-height-store", "data"),
    prevent_initial_call=False,
)
...
function(measureMode, measureHover, oscillatorRange, candleInfo, oscillatorInfo, oscillatorSyncInfo, extendX, focusEntry, figure, viewState, chartTaskId, chartUiState) {
    ...
    if (!effectiveOscillatorRange && window.clearMeasureOscillatorExportShapes) window.clearMeasureOscillatorExportShapes(plot);
    if (window.attachNativeMeasureOverlayListeners) window.attachNativeMeasureOverlayListeners(plot);
...
@app.callback(
    ...
    Output("chart-fast-navigation-payload", "children"),
    Input("chart-task-id", "data"),
...
def update_task_chart(task_id, chart_action, chart_event_context, force_full_render, indicator_order, main_full_height, osc_full_height, rsi_visible, stochastic_visible, volume_visible, adx_visible, macd_visible, disparity_visible, cci_visible, cmf_visible, cho_visible, atr_visible, cvd_visible, slr_visible, mfi_visible, vwap_visible, open_interest_visible, strategy_visible, impulse_visible, events_visible, focus_entry, candle_info_enabled, chart_request, chart_ui_state, chart_view_state, measure_mode, current_render_schema):
    ...
    if ctx.triggered_id == "chart-force-full-render-store":
        reason = force_full_render.get("reason") if isinstance(force_full_render, dict) else "unknown"
        interaction_trace(f"chart fast fallback full render task={task_id} reason={reason}")
    ...
    def add_cci_trace(target_fig, row):
        cci_values = df['cci_20']
        finite_values = cci_values[np.isfinite(cci_values)]
        cci_min = float(finite_values.min()) if len(finite_values) else -100.0
        cci_max = float(finite_values.max()) if len(finite_values) else 100.0
        add_hover_spike_bar(target_fig, row, cci_min, cci_max, f'_spike_hover_cci_{row}')
        target_fig.add_trace(make_chart_cci_trace(
            x=trace_x, y=cci_values, mode='lines', name='CCI 20',
            line=dict(color='#00838f', width=1.4), connectgaps=True,
            hovertemplate='CCI 20: %{y:.2f}<extra></extra>'
        ), row=row, col=1)
        target_fig.add_hline(y=100, line_dash="dash", line_color="red", row=row, col=1)
        target_fig.add_hline(y=-100, line_dash="dash", line_color="green", row=row, col=1)
    ...
    try:
        ...
        cache_key = ("chart_period_v5_stoch_300_alignment", start_ms, end_ms, source_stat.st_mtime_ns, source_stat.st_size)
    except OSError:
        cache_key = ("chart_period_v5_stoch_300_alignment", start_ms, end_ms)
    ...
    if stochastic_visible:
        stochastic_columns = {
            ...
            # Keep the chart identical to the dynamic strategy source:
            # 300/1/10 = lookback 300, %D 1, %K smoothing 10.
            ('stoch_k_300_1_10', 'stoch_d_300_1_10'): (300, 1, 10),
        }
    ...
    cci_cached = 'cci_20' in df.columns
    ...
    legacy_compact_count = sum(bool(value) for value in (
        rsi_visible, volume_enabled, adx_visible, macd_visible, disparity_visible,
    ))
    legacy_compact_height = (
        980 if stochastic_visible else
...
def evaluate_dynamic_checkup_path(path, stop_loss_pct, max_dd_pct, tp_levels, stop_rules, oscillator_exit_specs=None, oscillator_exit_window=1):
    ...
    exit_values = {
        spec.get("column"): path.get(spec.get("column"))
        for spec in selected_exit_specs
        if spec.get("column") in path
    }
    oscillator_exit_mask = (
        build_oscillator_specs_match_mask(
            exit_values, selected_exit_specs, len(path["highs"]),
            window=oscillator_exit_window, min_idx=0,
        )
        if (DYNAMIC_STOCH_MASK_VECTORIZATION_ENABLED
            and selected_exit_specs and close_values is not None) else None
    )
    ...
    for idx, (high, low) in enumerate(zip(path["highs"], path["lows"])):
        ...
        if selected_exit_specs and close_values is not None and idx > 0:
            if oscillator_exit_mask is not None:
                oscillator_ready = bool(oscillator_exit_mask[idx])
            else:
                oscillator_ready = oscillator_specs_met_within_window(
                    path, selected_exit_specs, idx, oscillator_exit_window, min_idx=0
                )
            if oscillator_ready:
...
def build_oscillator_specs_match_mask(values_by_column, specs, length, window=1, min_idx=0):
    """Vectorize the existing oscillator-window semantics without changing them.

    Each enabled condition is evaluated on the same current/previous values as
    ``oscillator_condition_met``. A rolling recent-match mask then reproduces
    ``oscillator_condition_met_within_window``. Keeping this as a separate
    helper makes parity easy to test and lets entry and exit scans avoid nested
    Python loops over every candle and oscillator.
    """
    length = max(0, int(length or 0))
    min_idx = max(0, min(int(min_idx or 0), length))
    window = max(1, int(window or 1))
    ...
    for spec in specs or []:
        ...
        values = np.asarray(values, dtype=float)[:length]
        valid = ~pd.isna(values)
        level = float(spec.get("level") if spec.get("level") is not None else 0.0)
        if condition == "above":
            matches = valid & (values >= level)
        elif condition == "below":
            matches = valid & (values <= level)
        else:
            previous = np.empty(length, dtype=float)
            previous[0] = np.nan
            if length > 1:
                previous[1:] = values[:-1]
            pair_valid = valid & ~pd.isna(previous)
            if condition == "cross_down":
                matches = pair_valid & (previous > level) & (values <= level)
            elif condition == "cross_up":
                matches = pair_valid & (previous < level) & (values >= level)
            else:
                matches = np.zeros(length, dtype=bool)

        # Truncate before min_idx before applying the window, exactly matching
        # max(min_idx, idx-window+1) in the scalar implementation.
        recent = np.zeros(length, dtype=bool)
        eligible = matches[min_idx:].astype(np.int8, copy=False)
        if len(eligible):
            cumulative = np.cumsum(eligible, dtype=np.int64)
            counts = cumulative.copy()
            if window < len(eligible):
                counts[window:] -= cumulative[:-window]
            recent[min_idx:] = counts > 0
        combined &= recent
        if not combined.any():
            break
    if not active_specs:
        combined[:min_idx] = False
    return combined
...
def build_oscillator_reversal_path_from_source(source, oscillator_specs, entry_condition_window=1):
    ...
    if DYNAMIC_STOCH_MASK_VECTORIZATION_ENABLED:
        oscillator_matches = build_oscillator_specs_match_mask(
            source["oscillator_values"], selected_specs, len(df),
            window=entry_condition_window, min_idx=cross_idx,
        )
        oscillator_positions = np.flatnonzero(oscillator_matches[cross_idx:])
        if len(oscillator_positions) == 0:
            return None
        oscillator_idx = cross_idx + int(oscillator_positions[0])
    else:
        oscillator_idx = next((
            idx for idx in range(cross_idx, len(df))
            if oscillator_specs_met_within_window(
                source["oscillator_values"], selected_specs, idx,
                entry_condition_window, min_idx=cross_idx,
            )
        ), None)
...
def run_oscillator_reversal_checkup(n_clicks, stoch14_level, stoch14_condition, stoch40_level, stoch40_condition, stoch60_level, stoch60_condition, stoch300_level, stoch300_condition, rsi_level, rsi_condition, down_stoch14_level, down_stoch14_condition, down_stoch40_level, down_stoch40_condition, down_stoch60_level, down_stoch60_condition, down_stoch300_level, down_stoch300_condition, down_rsi_level, down_rsi_condition, stop_loss_pct, max_dd_pct, tp_text, stop_rules_text, sl_grid_text, entry_condition_window, oscillator_exit_window, exit_enabled, exit_sell_stoch14_level, exit_sell_stoch14_condition, exit_sell_stoch40_level, exit_sell_stoch40_condition, exit_sell_stoch60_level, exit_sell_stoch60_condition, exit_sell_stoch300_level, exit_sell_stoch300_condition, exit_buy_stoch14_level, exit_buy_stoch14_condition, exit_buy_stoch40_level, exit_buy_stoch40_condition, exit_buy_stoch60_level, exit_buy_stoch60_condition, exit_buy_stoch300_level, exit_buy_stoch300_condition, notional_usd, round_trip_cost_pct, open_return_pct, task_limit_enabled, task_limit, _version):
    ...
    try:
        ...
        elapsed = time.time() - started
        interaction_trace(
            f"dynamic stochastic complete tasks={len(tasks)} elapsed={elapsed:.3f}s "
            f"condition_mode={'vectorized' if DYNAMIC_STOCH_MASK_VECTORIZATION_ENABLED else 'scalar'} "
        ...
        status = (
            ...
            f"condition matching={'vectorized' if DYNAMIC_STOCH_MASK_VECTORIZATION_ENABLED else 'scalar rollback'}; "
