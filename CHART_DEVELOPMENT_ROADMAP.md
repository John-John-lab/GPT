...
- Dynamic stochastic entry/exit condition windows are evaluated as vectorized
  boolean masks before the sequential trade simulation. The sequential
  stop/TP/exit loop remains authoritative because its ordering is path
  dependent. Chart and strategy both interpret Stoch 300/1/10 as lookback 300,
  `%D` length 1, and `%K` smoothing 10. Set
  `GPT_DYNAMIC_STOCH_VECTORIZED=0` for an immediate scalar-parity rollback.
