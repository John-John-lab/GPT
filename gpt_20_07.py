...
document.addEventListener('keydown', function(e) {
    if (e.key !== 'Backspace' || e.target.matches('input, textarea, [contenteditable="true"]')) return;
    const root = document.getElementById('task-chart');
    const plot = root ? (root.querySelector('.js-plotly-plot') || root) : null;
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Button("Candle Info: On", id="toggle-chart-info-box-btn", title="Toggle exact OHLC information for the main candle chart", style={
                                "background": "#fff8e1",
                                "color": "black",
                                "border": "1px solid #f9a825",
                                "padding": "6px 10px",
...
function(measureMode, measureHover, candleInfo, oscillatorInfo, extendX, figure, viewState, chartTaskId) {
    ...
    (figure.data || []).forEach(function(trace, index) {
        ...
        if (!showHover || isHelper) {
            ...
            hovertemplate = oscillatorInfo ? '%{x|%Y-%m-%d %H:%M}<extra></extra>' : '<extra></extra>';
        } else if (trace.type === 'candlestick' && !candleInfo) {
            hoverinfo = 'skip';
        } else if (trace.type !== 'candlestick' && !oscillatorInfo) {
            hoverinfo = 'skip';
